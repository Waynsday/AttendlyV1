...
if [[ "scripts/enhanced-attendance-sync.ts" -nt "scripts/enhanced-attendance-sync.js" ]] 2>/dev/null; then
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    if npx tsx scripts/enhanced-attendance-sync.ts "${cmd_args[@]}"; then
        end_time=$(date +%s)
