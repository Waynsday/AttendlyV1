-- =====================================================
-- Fix for Student Attendance Data Function Type Mismatch
-- =====================================================
-- This script fixes the type mismatch error:
-- "Returned type character varying(50) does not match expected type text in column 2"
-- 
...
RETURNS TABLE (
  id UUID,
  full_name TEXT,  -- Changed from VARCHAR(50) to TEXT
  grade_level INTEGER,
  current_homeroom_teacher TEXT,  -- Changed from VARCHAR(50) to TEXT
  aeries_student_id TEXT,  -- Changed from VARCHAR to TEXT
  attendance_rate NUMERIC,
  absent_days BIGINT,
  enrolled_days BIGINT,
  present_days BIGINT,
  risk_level TEXT,  -- Changed from VARCHAR to TEXT
  tardies INTEGER,
  school_name TEXT,  -- Changed from VARCHAR to TEXT
  total_count BIGINT
) 
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Return query with attendance calculations
  RETURN QUERY
...
-- =====================================================
-- Alternative: Create a View-Based Solution
-- =====================================================
-- If the function continues to have issues, create this view instead:
...
WITH student_stats AS (
  SELECT 
    ...
    s.grade_level,
    COALESCE(s.current_homeroom_teacher, 'Staff')::TEXT AS current_homeroom_teacher,
    s.aeries_student_id::TEXT,
    s.school_id,
  ...
  WHERE s.is_active = true
  GROUP BY s.id, s.first_name, s.last_name, s.grade_level, 
           s.current_homeroom_teacher, s.aeries_student_id, 
           s.school_id, sch.school_name
)
SELECT 
  *,
  CASE 
    WHEN attendance_rate >= 95 THEN 'low'::TEXT
    WHEN attendance_rate >= 90 THEN 'medium'::TEXT
    ELSE 'high'::TEXT
  END AS risk_level
FROM student_stats;

-- Grant select permissions on the view
GRANT SELECT ON student_attendance_view TO authenticated;
GRANT SELECT ON student_attendance_view TO service_role;